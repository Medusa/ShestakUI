...
local function Shared(self, unit)
	...
	if not (self:GetAttribute("unitsuffix") == "pet" or (self:GetAttribute("unitsuffix") == "target" and unit ~= "tank")) then
		...
		self.Power:SetPoint("BOTTOMLEFT", self, "BOTTOMLEFT", 0, -9)
		self.Power:SetPoint("BOTTOMRIGHT", self, "BOTTOMRIGHT", 0, -17)
		self.Power:SetPoint("TOP", self, "BOTTOM", 0, -7)
	...
	if C.raidframe.icons_role == true and not (self:GetAttribute("unitsuffix") == "target") then
		...
		self.LFDRole:SetPoint("BOTTOMRIGHT", self.Health, 3, -7)
	end
...
oUF:Factory(function(self)
	...
	if C.raidframe.show_party == true then
		...
		partytarget:SetPoint("TOPLEFT", party, "BOTTOMLEFT", 0, -16)
		...
		partypetupdate:SetScript("OnEvent", function(...)
			...
			partypet:SetPoint("TOPLEFT", party[lastGroup], "BOTTOMLEFT", 0, -((unit_height / 2) + 23))
		end)
		...
		if C.raidframe.raid_groups_vertical then
			...
			for i = 1, C.raidframe.raid_groups do
				...
				if i == 1 then
					...
					raidgroup:SetPoint("TOPLEFT", raid[i-1], "BOTTOMLEFT", 0, -16)
